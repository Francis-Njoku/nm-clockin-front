  ...
  return (
    <div className="dd-handle mt-2">
      <div className="task-info d-flex align-items-center justify-content-between">
        <h6 className={`${titleBGClass} py-1 px-2 rounded-1 d-inline-block fw-bold small-14 mb-0`}>
          {title}
        </h6>
        <div className="task-priority d-flex flex-column align-items-center justify-content-center">
          ...
          <span
            className={`badge ${status === 'MEDIUM' ? 'bg-warning' : status === 'High' ? 'bg-danger' : 'bg-success'} text-end mt-2`}>
            {status}
          </span>
        </div>
      </div>
