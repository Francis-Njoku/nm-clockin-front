...
function TaskNestable1(props) {
  ...
  const onDropT = (data) => {
    ...
    const filtereddropable = gragable.filter((item) => item.Data !== data.grag)
    const filteredCopmplete = Copmplete.filter((item) => item.Data !== data.grag)
    setDropable([...dropabl])
  ...
  const onDrop = (data) => {
    ...
    const filteredgragable = dropable.filter((item) => item.Data !== data.grag)
    const filteredCopmplete = Copmplete.filter((item) => item.Data !== data.grag)
    setGragable([...dropabl])
  ...
  const onDropCompete = (data) => {
    ...
    const filteredgragable = gragable.filter((item) => item.Data !== data.fruit)
    const filtereddropable = dropable.filter((item) => item.Data !== data.fruit)
    ...
    <div className="row taskboard g-3 py-xxl-4">
      <div className="col-xxl-4 col-xl-4 col-lg-4 col-md-12 mt-xxl-4 mt-xl-4 mt-lg-4 mt-md-4 mt-sm-4 mt-4">
        ...
        <Droppable
          ...
          {dropable.map((data, i) => {
            return (
              <Draggable type={data.type} data={data.Data} key={'gragglkasej' + i}>
